...
test("public pages load seeded data through Fastify and Postgres", async ({ page }) => {
  ...
  await expect(statusCards.locator(".stat-icon svg")).toHaveCount(6);
  ...
  await expect(statusCards.filter({ hasText: "Estimated Time Remaining" }).locator("[data-testid='stat-value']")).toHaveText("18h");
