...
try {
  for (const row of rows) {
    const title = row.game ?? row.title ?? row.Title;
    const serialNumber = row.serial ?? row.serial_number ?? row.SerialNumber;
    ...
    await pool.query(
      ...
      `,
      [game.rows[0].game_id, serialNumber, row.region ?? null, row.notes ?? "Imported from wiki serials"]
    );
  }
} finally {
  await pool.end();
